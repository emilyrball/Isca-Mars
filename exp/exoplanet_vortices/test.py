...
if __name__=="__main__":
    ...
    dust_clim = 'cdod_clim_scenario'

    dust_scale = 7.4e-5
    radius = float(sys.argv[1])
    obliq = float(sys.argv[2])
    solar_const = float(sys.argv[3])

    for conv in conv_schemes:
        for depth_val in depths:
            for per_value in pers:
                exp = Experiment('soc_mars_mk36_per_value'+str((per_value))+'_radius'+str(radius)+'_solar_const'+str(solar_const)+'_obliq'+str(obliq)+'_'+conv+'_mld_'+str(depth_val)+'_with_mola_topo_'+dust_clim+'_'+str(dust_scale)+'_lh_rel', codebase=cb)
                exp.clear_rundir()
                ...
                exp.namelist['constants_nml']['radius']   = radius * 10**6 
                exp.namelist['constants_nml']['solar_const'] = solar_const
                ...
                exp.namelist['astronomy_nml']['obliq'] = obliq
